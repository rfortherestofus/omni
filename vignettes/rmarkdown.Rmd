...
However, this first method will force you to make a new folder to put all of your files in, which is kind of annoying. I've created a function that gets around this. Just load the `omni` package using `library(omni)` and then type `omni_report()`. By default, this function creates a new RMarkdown document called report.Rmd, but you can change this to anything you want. You can also enter an argument in the function to change it that way. For example, type `omni_report("snazzy-report")` and you'll get an RMarkdown file called snazzy-report.Rmd.

When you type `omni_report()` you'll get a pop-up like this. Just hit save and then find the report.Rmd (or whatever location you chose) and start working.

![](https://p218.p3.n0.cdn.getcloudapp.com/items/4gumRNGK/Screen%20Recording%202020-03-22%20at%2002.21%20PM.gif)
