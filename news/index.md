...
- Added a manual fix for a heading that lands at the top of a printed
  ...
  CSS-authoring time. After knitting, wrap a heading that visibly lands
  at the top of a page in a fenced div with the new
  `remove-header-space` class and re-knit: `::: {.remove-header-space}`
  / `## My Heading` / `:::`. Only that heading’s top spacing is removed;
