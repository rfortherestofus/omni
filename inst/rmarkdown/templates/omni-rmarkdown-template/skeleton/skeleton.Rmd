...
```{r fig.height=3, fig.cap = "A very nice iris plot"}
iris %>%
  group_by(Species) %>%
...
```{r, fig.cap = "A random figure", out.width="200px", fig.align='center'}
...
mtcars %>% 
  slice(1:5) %>% 
  omni_table() %>% 
  save_as_image("test_flextable.png")
```

```{r, fig.cap="Table as image"}
include_graphics("test_flextable.png")
```

# An other first level header
