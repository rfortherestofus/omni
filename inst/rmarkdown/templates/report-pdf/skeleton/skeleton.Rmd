...
```{r, results='asis'}
library(omni)
omni_set_page_start_pdf(start_from = 1, counter_start = 10)
```
...
See Table \@ref(tab:work-areas).

```{r work-areas, tab.cap="Key activities and deliverables by work area."}
tibble::tribble(
  ~`Work Area`      ,
  ~`Key Activities` ,
  ~`Deliverables`   ,
...
::: {.appendix-pattern-only}

::: {.pattern-appendix}
![](https://github.com/rfortherestofus/omni/blob/main/inst/assets/images/pattern-appendix.png?raw=true)
:::
