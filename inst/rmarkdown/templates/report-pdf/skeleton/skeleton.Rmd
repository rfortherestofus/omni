---
title: "Report Title Goes Here:<br>2 Lines Max for Best Readability"
subtitle: "Sub-Headline Goes Here" 
client_name: "[client]"
client_city: "[city]"
client_state: "[state]"
report_year: "[year]"
acknowledgements: "[...]"
date: "`r toupper(format(Sys.time(), '%B %Y'))`"
toc-title: "Table of Contents"
knit: pagedown::chrome_print
output:
  omni::pdf_report:
    main_font: "Inter Tight"
    secondary_font: "Sometype Mono"
    background_color: "#f9f7f4"
    primary_color: "#081c39"
    remove_logo: FALSE
    remove_cover_page: TRUE
abstract: >
  ```{r, child = system.file('assets/rmd/abstract.Rmd', package = 'omni')}
  ```
---



```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)
```

```{=html}
<div class="page-break-before"></div>
```

```{r}
library(omni)
```


::: {.small-title .dot-red}
Small title section 
:::

## Example Section

### Subtopic A

The project will start with a virtual project alignment meeting with the key county leadership and staff to discuss the overall approach; review the initial workplan to support establishing strong working relationships; and ensure clear communication, detailed schedules, timelines and milestones. 

#### Subtopic 1

The project will start with a virtual project alignment meeting with the key county leadership and staff to discuss the overall approach; review the initial workplan to support establishing strong working relationships; and ensure clear communication, detailed schedules, timelines and milestones. 

This is the end of the page.

\newpage



::: {.small-title .dot-green}
Small title section 
:::

## Example Section


### Section title

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum



```{=html}
<div class="page-break-before"></div>

<section class="cover-page-full">
  <div class="cover-content">
    <h2>Cover Page Title</h2>
  </div>
</section>

<div class="page-break-after"></div>
```

::: {.small-title .dot-purple}
Small title section 
:::

## Chapter title

:::: {.columns}


::: {.column width="29%"}

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum

:::

::: {.column width="1%"}

:::

::: {.column width="70%"}

![](logo.png)

:::

::::


### Section title

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum

## Chapter title

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum


![](logo.png){.wrap-right}

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum


### Section title

![](logo.png){.wrap-left width="40%"}

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 

\newpage

### Section title

:::: {.columns}

::: {.column width="50%"}
Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 
:::

::: {.column width="50%"}

```{r}
callout_box(
  text = "This is a callout box. You can <highlight>change text color to highlight certain parts</highlight>, or just leave the text all white. Change the background color as desired to match the page.",
  color = "olive-green 600"
)
```

:::

::::



### First section

:::: {.columns}

::: {.column width="50%"}
Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 
:::

::: {.column width="50%"}
```{r}
quote_box(
  text = "This is a quote box. You can <highlight>change text color to highlight certain parts</highlight>, or just leave the text all white. Change the background color as desired to match the page.",
  author = "David Keyes",
  color = "olive-green 600"
)
```

:::

::::



\newpage

::: {.small-title}
Small title section 
:::

## Chapter title

:::: {.columns}

::: {.column width="50%"}

- Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore. 
  - Lorem ipsum dolor sit amet, consectetu.
- Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.
  - Ut enim ad minim veniam, quis nostru.
  - Nisi ut aliquip ex ea commodo consequat.

:::

::: {.column width="50%"}
![](https://images.unsplash.com/photo-1747599309107-20504ba6b8dd?q=80&w=1752&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D)
:::

::::

### Section title

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 

<hr>



Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 

#### Number Emphasis

```{r}
number_emphasis(
  number = 1234,
  text = "pt. Inter Tight for stats numbers. 12 pt Inter Tight for stats content.",
  color = "teal 400",
  
)
```


#### Icon Text Grid

```{r}
width_px <- 50
icon_color_fg <- "white"
icon_color_bg <- 'teal 600'

icon_text_grid(
  omni_icon('education', width_px, icon_color_bg, icon_color_fg),
  icon_text(
    text = '**71.9%:** Any person younger than 21 caught with or suspected of consuming alcohol or marijuana is charged with Minor in Possession (MIP).'
  ),
  omni_icon('security', width_px, icon_color_bg, icon_color_fg),
  icon_text(
    '**69.8%:** Adults in Colorado who knowingly hep someone younger than 18 break the law - which includes providing minors with alcohol or drubgs - can be charged with a Class 4 felony.'
  ),
  omni_icon('vault', width_px, icon_color_bg, icon_color_fg),
  icon_text(
    '**34.4%:** It is legal for a person ages 18-20 to possess marijuana with a medical marijuana card.'
  ),
  width_px = width_px
)
```


#### Sub section title

Lorem Ipsum

<div class="page-break-before"></div>

## Appendix A

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum

## Appendix B

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum


<script>
if (typeof Paged !== 'undefined') {
  class CustomPagedHandler {
    afterRendered(pages) {
      var covers = document.querySelectorAll('.cover-page-full');

      covers.forEach(cover => {
        var page_container = cover.closest('.pagedjs_page');
        if (page_container) {
          var marginBottom = page_container.querySelector('.pagedjs_margin-bottom');
          if (marginBottom) {
            marginBottom.classList.add('custom-style');
          }

          var bottomCenter = page_container.querySelector('.pagedjs_margin.pagedjs_margin-bottom-center.hasContent');
          if (bottomCenter) {
            bottomCenter.classList.add('custom-style');
          }
        }
      });
    }
  }

  Paged.registerHandlers(CustomPagedHandler);
}

</script>