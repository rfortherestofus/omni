  ...
  class extends Paged.Handler {
    ...
    retrieveUniquePrefixes(content) {
      const els = content.querySelectorAll(
        ".figure > img[data-prefix] + p.caption, .figure > p.caption + img[data-prefix]"
      );
      // retrieve all prefixes
      let prefixes = Array.from(els).map(
        (el) => el.parentNode.querySelector("img").dataset.prefix
      );
      // get unique prefixes
    ...
    beforeParsed(content) {
      ...
      for (const prefix of this.prefixes) {
        const els = content.querySelectorAll(
          '.figure > img[data-prefix="' +
            prefix +
            '"] + p.caption, .figure > p.caption + img[data-prefix="' +
            prefix +
            '"]'
        );
        for (let i = 0; i < els.length; i++) {
          ...
          lofEntry.childNodes[0].textContent =
            lofEntry.childNodes[0].textContent.replace(
              /\d*.\d*/,
              newLabel + ":"
            );
        }
