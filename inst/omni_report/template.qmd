---
title: "Report Title Goes Here:<br>2 Lines Max for Best Readability"
subtitle: "Sub-Headline Goes Here"
secondary-subtitle: "Optional Other Sub-Headline Goes Here"
author: "Author"
date: "`r toupper(format(Sys.time(), '%B %Y'))`"
execute:
    warning: false
    message: false
    echo: false
format:
    # omni_report-html:
    #   use-csi-style: false
    #   contact-email: projects@omni.org
    omni_report-typst:
      cover-page: true
      title-page: true
      cover-pattern: pattern-cover-01-yellow
      use-csi-style: false
      client-name: "[Favorite client]"
      client-city: "[city]"
      client-state: "[state]"
      contact-email: projects@omni.org
      report-year: "[year]"
      acknowledgements: "**[one name, and another name, and more names, and lots of names, and even more names. There were lots of people involved with this magnificient project.]**"
      start-page-number: 1
---




```{r, echo=FALSE}
library(tidyverse)
library(janitor)
library(omni)
library(scales)
```


# Key Findings {.pattern-01-yellow}

## First section

- Bullet 1
- Bullet 2
    - bullet 1
    - bullet 2
        - bullet 1
        - bullet 2

Lorem Ipsum is simply dummy **text of the printing and typesetting** industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.

Lorem Ipsum is simply [dummy text]{.plum-200}

[Lorem Ipsum **is simply dummy** text]{.olive-green-400}

Lorem [Ipsum]{.orange-red-600} is simply dummy text.


:::: {.columns}

::: {.column width="55%"}
Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.
:::

::: {.column width="2%"}
:::

::: {.column width="43%"}

Some text

```{r, echo=FALSE}
omni::quote_box(
  text = "This is a quote box. You can <highlight>change text color to highlight certain parts</highlight>, or just leave the text all white. Change the background color as desired to match the page.",
  author = "David Keyes",
  color = "olive-green-600"
)
```

:::

::::

### Subtopic A

```{r, echo=FALSE}
#| fig-alt: "Scatterplot of sepal length versus sepal width for the iris dataset, colored by species. Setosa points cluster at short, wide sepals; versicolor and virginica overlap at longer, narrower sepals, with virginica trending longest."
library(tidyverse)

ggplot(data = iris, aes(x = Sepal.Length, y = Sepal.Width, color = Species)) +
  geom_point() +
  labs(
    title = "Sepal Length vs Sepal Width",
    x = "Sepal Length (cm)",
    y = "Sepal Width (cm)"
  ) +
  theme_minimal()
```

### Tables and cross-references

This template is written in Quarto, not R Markdown, and Quarto uses its own cross-referencing syntax. In R Markdown you would name a chunk and give it a `tab.cap` option (e.g. `{r fruit-sales, tab.cap="..."}`) and reference it with `\@ref(tab:fruit-sales)`. In Quarto, chunk options move inside the chunk as `#|` lines, a table's chunk gets a `label` starting with `tbl-`, and you reference it in text with `@tbl-fruit-sales` (no `\@ref()`, no backslash), as in "See @tbl-fruit-sales." below. The same pattern applies to figures (`fig-` labels, referenced with `@fig-`), equations, and sections. See the [Quarto cross-references guide](https://quarto.org/docs/authoring/cross-references.html) for the full syntax.

**Note:** when a table is produced with `omni::omni_tinytable()`, you must set both `label` and `tbl-cap` in the chunk options. If `tbl-cap` is set without a `label` starting with `tbl-`, tinytable will silently drop the caption.

See @tbl-fruit-sales.

```{r}
#| label: tbl-fruit-sales
#| tbl-cap: "Quarterly sales of nonsense fruit."
tibble::tribble(
  ~Fruit        , ~`Units Sold` , ~`Revenue (USD)` ,
  "Apples"      ,          1240 ,             3100 ,
  "Bananas"     ,           980 ,             1470 ,
  "Cherries"    ,           310 ,             2480 ,
  "Dragonfruit" ,            75 ,              900
) |>
  omni::omni_tinytable()
```


Here is a footnote reference,[^1] and another.[^longnote]

[^1]: Here is the footnote.

[^longnote]: Here's the other one

This paragraph won't be part of the note, because it isn't indented.

<hr>

# Recommendations {.pattern-08-plum}

## Second section

### Subtopic A

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.


### Subtopic B

Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop pub

### Subtopic C


:::: {.columns}

::: {.column width="55%"}
Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 
:::


::: {.column width="45%"}

```{r, echo=FALSE}
omni::callout_box(
  text = "This is a callout box. You can <highlight>change text color to highlight certain parts</highlight>, or just leave the text all white. Change the background color as desired to match the page.",
  color = "orange-red-600"
)
```

:::

::::

Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum. 
Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum 

# Here is one more colored page break with a long title to see how the headline behaves when long {.pattern-07-olive}

## And some section


### Just to have...

### ...the table of contents not look silly

# A section break without a pattern

A level 1 heading without a `.pattern-XX` class still starts a new page and still appears in the table of contents, but without the full-page colored divider.

## Additional Design Elements

Again, to see how these are made, take a look at the R Markdown document.

::: {.chapter-dot .dot-teal-600}
Optional chapter label
:::

### Chapter Dot

An opt-in "eyebrow" label with a colored dot bullet, placed above a heading
via a `.chapter-dot .dot-<color>[-<shade>]` fenced div. See the README for
the full list of color/shade classes.

#### Number Emphasis

```{r, echo=FALSE}
number_emphasis(
  number = 1234,
  text = "pt. Inter Tight for stats numbers. 12 pt Inter Tight for stats content.",
  color = "teal-400"
)
```



```{r, echo=FALSE}
number_emphasis(
  number = 1234,
  text = "is a pretty number that deserves a bit of text to describe how nice it is and how it should be emphasized *a lot*.",
  color = "teal-400",
  font_size_pt = 16,
  text_font_size_pt = 8
)
```


#### Icon Text Grid

```{r, echo=FALSE}
width_px <- 50
icon_color_fg <- "white"
icon_color_bg <- 'teal-600'

icon_text_grid(
  omni_icon('education', width_px, icon_color_bg, icon_color_fg),
  icon_text(
    text = '**71.9%:** Any person younger than 21 caught with or suspected of consuming alcohol or marijuana is charged with Minor in Possession (MIP).'
  ),
  omni_icon('security', width_px, icon_color_bg, icon_color_fg),
  icon_text(
    '**69.8%:** Adults in Colorado who knowingly hep someone younger than 18 break the law - which includes providing minors with alcohol or drubgs - can be charged with a Class 4 felony.'
  ),
  omni_icon('vault', width_px, icon_color_bg, icon_color_fg),
  icon_text(
    '**34.4%:** It is legal for a person ages 18-20 to possess marijuana with a medical marijuana card.'
  ),
  width_px = width_px
)
```



## Appendix A {.appendix}

Lorem Ipsum is simply dummy **text of the printing and typesetting** industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.


## Appendix B {.appendix}


Lorem Ipsum is simply dummy **text of the printing and typesetting** industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.

Lorem Ipsum is simply dummy **text of the printing and typesetting** industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.

Lorem Ipsum is simply dummy **text of the printing and typesetting** industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.
